...
int main(int argc, char ** argv) {
    ...
    ret = moonshine_streaming_encoder(ctx, features, feat_seq_len, feat_hidden,
                                       &encoded, &enc_seq_len, &enc_hidden);
    if (ret != 0) {
    ...
    printf("Encoder output: [%d, %d] (hidden x seq_len)\n", enc_hidden, enc_seq_len);

    // Print statistics
    float min_val = encoded[0], max_val = encoded[0], sum = 0;
    for (int i = 0; i < enc_hidden * enc_seq_len; i++) {
        if (encoded[i] < min_val) min_val = encoded[i];
        if (encoded[i] > max_val) max_val = encoded[i];
        sum += encoded[i];
    }
    float mean = sum / (enc_hidden * enc_seq_len);
    printf("Stats: min=%.4f max=%.4f mean=%.4f\n", min_val, max_val, mean);

    // Print first few values
    printf("First 10 values: ");
    for (int i = 0; i < 10 && i < enc_hidden * enc_seq_len; i++) {
        printf("%.6f ", encoded[i]);
    }
    printf("\n");

    // Dump to binary file for comparison with Python
    if (dump_path) {
        FILE * f = fopen(dump_path, "wb");
        if (f) {
            int32_t dims[2] = { enc_hidden, enc_seq_len };
            fwrite(dims, sizeof(int32_t), 2, f);
            fwrite(encoded, sizeof(float), enc_hidden * enc_seq_len, f);
            fclose(f);
            printf("Dumped encoder output to %s\n", dump_path);
        }
    }
